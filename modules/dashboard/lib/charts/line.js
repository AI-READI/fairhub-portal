...
class LineChart extends Chart {
  ...
  constructor(config) {
    ...
    self.projection = Object.hasOwn(config, "projection")
      ? config.projection
      : undefined;
    ...
    */
    ...
    self.x =
      self.projection !== undefined
        ? D3.scaleLinear()
            .domain(self.projection.xDomain)
    ...
    self.y =
      self.projection !== undefined
        ? D3.scaleLinear()
            .domain(self.projection.yDomain)
    ...
    */
    ...
    self.lineseries = self.lines
      ...
      .attr(
        "id",
        ([group, , filter]) =>
          `${self.setID}_line-series_${self.tokenize(group)}_${self.tokenize(
            filter,
          )}`,
      )
      .attr("fill", "none")
    ...
    self.pointseries = self.points
      ...
      .attr(
        "id",
        ([group, , filter]) =>
          `${self.setID}_point-series_${self.tokenize(group)}_${self.tokenize(
            filter,
          )}`,
      )
  ...
  clear() {
    ...
    if (self.Tooltip !== undefined && self.Tooltip !== null)
      self.Tooltip.clear();
    if (self.Filters !== undefined && self.Filters !== null)
      self.Filters.clear();
...
*/
  ...
  mapData(data, filter) {
    ...
    data = data.filter(
      (datum) => datum[self.accessors.x.key] !== "Value Unavailable",
    );
      ...
      return {
        ...
        filterby:
          self.selectedFilter !== "All"
            ? datum[self.accessors.filterby.key]
            : "All",
