...
class Legend extends Interface {
  constructor(config) {
    ...
    self.colors = self.items
      ...
      .attr(
        "id",
        (d) => `${self.setID}_legend-color_${self.tokenize(d[self.accessor])}`,
      )
      .attr("top", self.position[self.hposition])
      .attr(
        "left",
        (d, i) => self.position[self.vposition] + i * (self.itemsize + 7) - 5,
      )
    ...
    self.labels = self.items
      ...
      .attr(
        "id",
        (d) => `${self.setID}_legend-label_${self.tokenize(d[self.accessor])}`,
      )
      .style("text-transform", "capitalize")
  ...
  mouseOverLegend(e, d) {
    ...
    D3.selectAll(
      `[data-legend="${self.getPrefix}_${self.tokenize(d[self.accessor])}"]`,
    )
      .transition()
