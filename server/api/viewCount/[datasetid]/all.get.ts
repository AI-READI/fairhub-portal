export default defineEventHandler(async (event) => {
  ...
  for (const version of allVersions) {
    const res = await fetch(
      `https://umami.fairdataihub.org/api/websites/${process.env.UMAMI_WEBSITE_ID}/pageviews?unit=year&endAt=${currentTime}&startAt=1709149073000&filters[path]=/datasets/${version.id}&timezone=America/Los_Angeles`,
      {
        headers: {
